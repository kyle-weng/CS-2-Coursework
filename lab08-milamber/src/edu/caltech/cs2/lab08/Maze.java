...
public class Maze {
    ...
    // First one is the point itself, second is if we've been there
    ...
    public Point[] getChildren(Point point) {
        ...
        Point[] ret = new Point[counter];
        counter = 0;
        if (!north[point.x][point.y]) {
            Point toAdd = new Point(point.x, point.y + 1);
            toAdd.parent = point;
            ret[counter] = toAdd;
            counter++;
        }
        if (!east[point.x][point.y]) {
            Point toAdd = new Point(point.x + 1, point.y);
            toAdd.parent = point;
            ret[counter] = toAdd;
            counter++;
        }
    ...
    private void solveDFSRecursive(Point point) {
        ...
        for (Point next : children) {
            if (!done) {
                String representation = "(" + next.x + ", " + next.y + ")";
                if (!next.isEqual(point.parent)) {
                    solveDFSRecursive(next);
                }
            }
        }
    }
