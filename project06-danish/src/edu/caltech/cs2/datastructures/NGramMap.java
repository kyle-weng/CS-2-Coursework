...
public class NGramMap {
    ...
    public IPriorityQueue.PQElement<String>[] getCountsAfter(NGram ngram) {
        IDictionary<IterableString, Integer> wordToCount = map.get(ngram);

        // not a valid phrase in map
        if(wordToCount == null) {
            return null;
        }

        IPriorityQueue.PQElement<String>[] arr = new IPriorityQueue.PQElement[wordToCount.size()];
        int i = 0;
        for(IterableString word: wordToCount) {
            arr[i] = new IPriorityQueue.PQElement(word.toString(), wordToCount.get(word));
            i++;
        }
        return arr;
    ...
    public String[] getWordsAfter(NGram ngram, int k) {
        ...
        String[] topK = new String[Math.min(k, countsAfter.length)];
        TopKSort.sort(countsAfter, k);
        for(int i = 0; i < topK.length; i++) {
            topK[i] = countsAfter[i].data;
        }
        return topK;
    }
}
